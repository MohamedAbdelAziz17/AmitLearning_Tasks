...
uint32_t Timer0_Num_OVF = 0;
uint32_t Timer0_Num_CTC = 0;
uint32_t Timer0_Initial_Value = 0;  // used in ISR function

void (*Timer0_g_ptr_to_func)(void) = NULL_PTR;
...
{
	Timer0_g_ptr_to_func = ptr_to_func;	//set user's function as ISR
	
#if WAVE_GENERATION_MODE == NORMAL_MODE

	uint8_t Tick = PRESCALER / F_OSC;
	uint32_t Counts = (Delay * MISECOND) / Tick;		//	us / us
	Timer0_Num_OVF = Counts / BITS;	
	Timer0_Initial_Value = BITS - (Counts % BITS); //compensate counting the decimal value by pre-loading
	TCNT0 = Timer0_Initial_Value; // set data register to initial value
	Timer0_Num_OVF++; // must increased by one because the decimal value
...
ISR(TIMER0_OVF_vect)
{
	...
	static uint32_t Timer0_desired_counts = 0;
	
	Timer0_desired_counts++;
	
	if(Timer0_desired_counts == Timer0_Num_OVF && Timer0_g_ptr_to_func != NULL_PTR)
	{

		(*Timer0_g_ptr_to_func)();
		
		Timer0_desired_counts = 0;
...
ISR(TIMER0_COMP_vect)
{
	...
	if(desired_counts == Timer0_Num_CTC && Timer0_g_ptr_to_func != NULL_PTR)
	{
		(*Timer0_g_ptr_to_func)();
